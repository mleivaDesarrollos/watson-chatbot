(function(){
  ...
  var tooltip  = document.querySelector("#chat-circle");
  tooltip.setAttribute("datatoggle","tooltip");
  tooltip.setAttribute("data-placement","left");
  tooltip.setAttribute("title","Tenes un nuevo mensaje");
  
  $("#chat-circle").tooltip("show");
  ...
  startConversation();
  // // Separamos el botón para enviar
  // var btnToSend = document.querySelector('#btnSend');    
  // btnToSend.addEventListener('click', sendMessageEvent);
  // // Recolectamos el input para anexarle el enter
  // var iptMessage = document.querySelector('input[name="inpMessage"]');
  // iptMessage.addEventListener('keyup', e => {
  //     if(e.keyCode === 13) btnToSend.click();
  // });

  // iptMessage.focus();
  // Agregamos un evento de click
  // Evento click para solicitudes por GET, prueba inicial para envio de un mensaje
  // btnToSend.addEventListener('click', (e) =>{
  //     // Detenemos las acciones por defecto del botón
  //     e.preventDefault();
  //     // Obtenemos el input con el mensaje
  //     let iptMessage = document.querySelector('input[name="inpMessage"]');
  //     // Obtenemos el valor de la variable
  //     let messageToSend = iptMessage.value;
  //     // Ejecutamos un llamado asincrono con el mensaje que será enviado a watson
  //     AjaxCall({url:'/send?message=' + messageToSend, method: 'get', callback: RenderResponse})        
  // });
}());